  ...
  async hideOverlay() {
    ...
    for (let s of seleOver) {
      if (["a.gmoam_close_button"].indexOf(s) > -1) {
        ...
        if (await this.silentIsExistEle(iSele[0], true, 15000)) {
          let iframe = await this.getEles(iSele[0], 1000);
          await this.driver.switchTo().frame(iframe[0]); // 違うフレームなのでそっちをターゲットに
          if (await this.silentIsExistEle(s, true, 1500)) {
...
class PartsQuizDailyCommon extends BaseWebDriverWrapper {
  para;
  constructor(para) {
    super(para.isMob);
    this.para = para;
...
exports.PartsQuizDailyCommon = PartsQuizDailyCommon;
